 title:Swift中的面向协议编程：是否优于面向对象编程？
 date:2018/08/15
 tags:[Swift]
 categories: [appcoda]
...
本文，我们将探讨为什么 Swift 被认为是一门“面向协议”的语言；对比面向协议编程（POP）和面向对象编程（OOP）；对比“值语义”和“引用语义”；讨论“局部推理”；用协议实现代理模式；用协议代替类型；利用到协议的多态性；重审我的面向协议的实际代码；最终讨论为什么我没有100%使用 POP 编程。

> 关于WWDC链接的一点说明
> 在这一系列关于POP的两篇文章中，我至少添加了三个 Apple Worldwide Developers Conference (WWDC) 视频的链接。在 Safari 中点击这些链接将直接跳转至视频中的具体小节（并经常会从该处开始播放视频）。如果你并非使用 Safari，你将需要浏览视频，手动跳转至某一小节，或者查看该视频的文字版本。
...
在 Swift 之前，协议在 iOS 中就已经十分重要。回想我对长久以来，iOS 开发者们一直在使用的 `UITableViewDataSource` 和 `UITableViewDelegate` 的[讨论](https://appcoda.com/protocol-oriented-programming/)。再想一想你每天写的 Swift 代码。

用 Swift 编程的时候，不可能不利用到[标准库](http://swiftdoc.org)中的协议。例如，`Array` （一个继承了[10个协议](http://swiftdoc.org/v3.1/type/Array/hierarchy/)的 `struct`），`Bool` （一个继承了[7个协议](http://swiftdoc.org/v3.1/type/Bool/hierarchy/)的 `struct`），`Comparable` （一个[继承自另一个协议的协议，并且是很多其他 Swift 类型的继承先祖](http://swiftdoc.org/v3.1/protocol/Comparable/hierarchy/)），以及 `Equatable` （一个[很多 Swift 协议和类型的继承先祖](http://swiftdoc.org/v3.1/protocol/Equatable/hierarchy/)）。
...
在 Swift 中使用 POP 还有一个好处：不仅仅是类，值类型，比如 `struct` 和 `enum` 也可以遵循协议。我们在下面将会讨论使用值类型的一些优点。
...
这当然对所有使用值语义的数据类型都是适用的。我强烈建议你下载并完整阅览整个playground。
...
> Local reasoning 意味着，当你看你面前的代码的时候，你不需要去思考，剩下的代码怎样去和这个函数进行交互。也许你之前已经有过这种感觉。例如，当你刚加入一个新的团队，有大量的代码要去看，同时上下文的信息也非常匮乏，你能明白某一个函数的作用吗？做到这样的能力很重要，因为能够使得维护代码、编写代码、测试代码变得更加容易。

哈哈，你曾经有过这种感觉吗？我曾经读过一些其他人写的真的很好的代码。我曾经写过一些易读性非常好的代码。说实话，在30年的工作经验中，我要去支持和升级的绝大部分现存的代码都不会给我这种感觉。相反，我经常会变得非常困惑，因为当我看一段代码的时候，我往往对这段代码的作用毫无头绪。
...
代理模式是iOS中广泛使用的模式，其中一个必需的组成部分就是协议。在这里我们不需要再去重复。你可以阅读我该主题的 AppCoda [博客](https://www.appcoda.com/swift-delegate/)
...
我选择向你展示如何去扩展，或者说是延伸，随便哪种说法，`UIKit` 的类，因为 1) 你很可能非常习惯使用它们 2) 扩展 iOS SDK 中的类，比如 `UIView`，是比用你自己的类更加困难一些的。
...
这种继承多个其他协议的做法可以让开发者对现有代码增加新的功能，而且不会因为太多额外不相关的功能对代码造成”污染“或”臃肿“。这些协议中，每一个都是独立的。如果是作为类被添加到继承层级中的话，根据它们所处的位置，这些功能将会至少自动被其他一些类继承。
...
在 WWDC 有关 POP 的视频之一中，一位工程师/讲师说[”在 Swift 中我们有一种说法，不要从一个类开始写代码，从一个协议开始“](https://developer.apple.com/videos/play/wwdc2015-408/?time=882)。大概是的吧。这家伙开始了有关如何使用协议来写一个二分查找的冗长的讨论。我有点怀疑，这是不是我许多读者印象最深的部分。看完你失眠了吗？

这有点像是为了寻找一个 POP 解决方案而人为设计出的一个问题。也许问题是实际的，也许这种解决方案有优点，我也不知道。我的时间很宝贵，没有时间浪费在这种象牙塔理论上。如果读懂一段代码需要超过5分钟的时间，我就觉得这段代码违背了 Apple 的 ”local reasoning“ 原则。
