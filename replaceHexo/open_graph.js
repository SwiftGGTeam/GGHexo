...
function extractPageDescription(content) {
  if (!content) return ''; 
  var reg = new RegExp('SEO-网页描述=(.*)');
  var temp = content.match(reg);
  var description = temp ? temp[1].trim() : '';
  content.replace(reg, "");
  return description;
}
...
function openGraphHelper(options){
  ...
  var description = options.description || page.description || extractPageDescription(page.content) || page.excerpt || content || config.description;
